...
# -*- encoding: utf-8 -*-
'''
@File    :   manager_torch.py
@Time    :   2023/09/13 10:22:07
@Author  :   wangru
@Version :   1.0
'''
...
if check_gpus():
    ...
    class GPUManager():
        ...
        def chose_available_gpus(self, free_ratio=0.8, n_gpu=1):
            for old_infos,new_infos in zip(self.gpus,query_gpu(self.qargs)):
                old_infos.update(new_infos)
            unspecified_gpus=[gpu for gpu in self.gpus if not gpu['specified']] or self.gpus
            sort_gpu = self._sort_by_memory(unspecified_gpus, False)
            chosen_gpu = []
            for d in sort_gpu:
                if d['memory.free'] / d['memory.total'] >= free_ratio:
                    chosen_gpu.append(d['index'])
            return ','.join(chosen_gpu[0: n_gpu])
else:
    raise ImportError('GPU available check failed')

if __name__ == '__main__':
    gm = GPUManager()
    idx = gm.chose_available_gpus()
